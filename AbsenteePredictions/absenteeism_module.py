...
class absenteeism_model():
        ...
        def load_and_clean_data(self, data_file):
            ...
            rsn_1 = reason_columns.loc[:,1:14].max(axis=1)
            rsn_2 = reason_columns.loc[:,15:17].max(axis=1)
            rsn_3 = reason_columns.loc[:,18:21].max(axis=1)
            rsn_4 = reason_columns.loc[:,22:].max(axis=1)
            ...
            df = pd.concat([df, rsn_1, rsn_2, rsn_3, rsn_4], axis = 1)
            ...
            column_names = ['date', 'transport_exp', 'dist_to_work', 'age',
                           'avg_daily_work_load', 'bmi', 'education', 'children',
                           'pets', 'Absenteeism Time in Hours', 'rsn_1', 'rsn_2', 'rsn_3', 'rsn_4']
            df.columns = column_names

            # re-order the columns in df
            column_names_reordered = ['rsn_1', 'rsn_2', 'rsn_3', 'rsn_4', 'date', 'transport_exp', 'dist_to_work',
                                      'age', 'avg_daily_work_load', 'bmi', 'education', 'children',
            ...
            # re-order the columns in df
            column_names_upd = ['rsn_1', 'rsn_2', 'rsn_3', 'rsn_4', 'month_value', 'Day of the Week',
                                'transport_exp', 'dist_to_work', 'age', 'avg_daily_work_load', 'bmi',
            ...
            df = df.drop(['Day of the Week','avg_daily_work_load','dist_to_work'],axis=1)
        ...
        def predicted_outputs(self):
            if (self.data is not None):
                self.preprocessed_data['Probability'] = self.clf.predict_proba(self.data)[:,1]
                self.preprocessed_data ['Prediction'] = self.clf.predict(self.data)
